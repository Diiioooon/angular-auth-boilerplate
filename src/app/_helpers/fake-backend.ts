...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    return handleRoute();
        ...
        function generateJwtToken() {
            const account = accounts.find(x => x.email === body?.email) || accounts[accounts.length - 1];
            const tokenPayload = { 
        ...
        function generateRefreshToken() {
            ...
            document.cookie = `fakeRefreshToken=${token}; expires=${new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toUTCString()}; path=/`;
            return token;
        }

        function alertService(message: string) {
            const alertDiv = document.createElement('div');
            alertDiv.style.cssText = 'position:fixed;top:10px;right:10px;z-index:9999;background:#d1ecf1;border:1px solid #bee5eb;padding:15px;border-radius:5px;max-width:400px;';
